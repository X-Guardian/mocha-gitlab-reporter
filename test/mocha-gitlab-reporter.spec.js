...
function runTests(reporter, options, callback) {
  ...
  suite1.addTest(
    createTest('can behave like a flandip', { duration: 30003 }, function (done) {
...
describe('mocha-junit-reporter', function () {
  ...
  it('respects `[suiteFilename]` pattern in test results report filename', function (done) {
    ...
    runTests(reporter, function () {
      verifyMochaFile(
        reporter.runner,
        dir + 'results.' + (reporter._testsuites[0]?.testsuite[0]?._attr?.file ?? 'suiteFilename') + '.xml'
      );
      done();
    });
  });
  ...
  it('respects `[suiteName]` pattern in test results report filename', function (done) {
    ...
    runTests(reporter, function () {
      verifyMochaFile(
        reporter.runner,
        dir + 'results.' + (reporter._testsuites[1]?.testsuite[0]?._attr?.name ?? 'suiteName') + '.xml'
      );
      done();
    });
  });
  ...
  it('properly outputs tests when error in beforeAll', function (done) {
    ...
    runRunner(reporter.runner, function () {
      ...
      expect(reporter._testsuites[2].testsuite[1].testcase[0]._attr.name).to.equal('test 2');
      done();
    });
  });
  ...
  it('properly diffs errors from Chai', function (done) {
    ...
    runRunner(reporter.runner, function () {
      ...
      expect(reporter._testsuites[1].testsuite[1].testcase).to.have.lengthOf(2);
      expect(reporter._testsuites[1].testsuite[1].testcase[0]._attr.name).to.equal('test 1');
      expect(reporter._testsuites[1].testsuite[1].testcase[1].failure._attr.message).to.equal(
        'expected {} to deeply equal { missingProperty: true }'
      );
      expect(reporter._testsuites[1].testsuite[1].testcase[1].failure._cdata).to.match(
  ...
  describe('when "outputs" option is specified', function () {
    it('adds output/error lines to xml report', function (done) {
      ...
      runRunner(reporter.runner, function () {
        ...
        expect(reporter._testsuites[1].testsuite[1].testcase[0]._attr.name).to.equal(test.title);
        expect(reporter._testsuites[1].testsuite[1].testcase[1]).to.have.property('system-out', 'hello\nworld');
        expect(reporter._testsuites[1].testsuite[1].testcase[2]).to.have.property(
          'system-err',
  ...
  describe('when "attachments" option is specified', function () {
    it('adds attachments to xml report', function (done) {
      ...
      runRunner(reporter.runner, function () {
        ...
        expect(reporter._testsuites[1].testsuite[1].testcase[0]._attr.name).to.equal(test.title);
        expect(reporter._testsuites[1].testsuite[1].testcase[1]).to.have.property(
          'system-out',
          '[[ATTACHMENT|' + filePath + ']]'
        );
  ...
  describe('GitLab format', function () {
    it('generates GitLab compatible classnames and test names', function (done) {
      ...
      runRunner(reporter.runner, function () {
        ...
        expect(reporter._testsuites[2].testsuite[1].testcase[0]._attr.name).to.equal('test_invalid_password');
        expect(reporter._testsuites[2].testsuite[1].testcase[0]._attr.classname).to.equal(
          'Authentication Tests.LoginTest'
        );

        expect(reporter._testsuites[3].testsuite[1].testcase[0]._attr.name).to.equal('test_session_cleanup');
        expect(reporter._testsuites[3].testsuite[1].testcase[0]._attr.classname).to.equal(
          'Authentication Tests.LogoutTest'
        );
    ...
    it('uses dot separator for testcase classname', function (done) {
      ...
      runRunner(reporter.runner, function () {
        ...
        expect(reporter._testsuites[3].testsuite[1].testcase[0]._attr.classname).to.equal('Level 1.Level 2.Level 3');
    ...
    it('generates well-formed XML', function (done) {
      ...
      runRunner(reporter.runner, function () {
        ...
        expect(reporter._xml).to.include('classname="Authentication Tests.LoginTest"');
    ...
    it('converts absolute file paths to relative paths', function (done) {
      ...
      const absolutePath = path.join(process.cwd(), 'test', 'example.js');
    ...
    it('converts inherited absolute file paths to relative paths', function (done) {
      ...
      const suite1 = Suite.create(rootSuite, 'Test Suite');
      const absolutePath = path.join(process.cwd(), 'test', 'suite.spec.js');
